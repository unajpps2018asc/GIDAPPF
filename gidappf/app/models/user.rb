  ...
  def asigned_in_time_sheet?(time_sheet)
    ...
    unless self.documents.empty? then
      ...
      unless profile_key_hour_from.profile_values.empty? then
        if profile_key_hour_from.key.eql?(ProfileKey.find(24).key) && # si es mayor o igual a 'Elección de turno desde[Hr]:'
          ...
          out = self.usercommissionroles.last.commission.time_sheets.include?(time_sheet)
